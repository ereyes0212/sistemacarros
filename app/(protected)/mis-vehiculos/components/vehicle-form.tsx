...
export function VehicleForm({ data, vehicle }: { data: any; vehicle?: any }) {
  ...
  return <Card className="mx-auto max-w-4xl border-cyan-100/70 shadow-sm"><CardHeader><CardTitle>{vehicle ? "Editar vehículo" : "Publicar vehículo"}</CardTitle></CardHeader><CardContent>
    <form action={saveVehicle} className="grid gap-5 md:grid-cols-2">
      ...
      <div className="space-y-2"><Label>Marca</Label><select name="brandId" value={selectedBrandId} onChange={(event) => { setSelectedBrandId(event.target.value); setSelectedModelId(""); }} required className="h-10 w-full rounded-md border bg-background px-3 text-sm">{data.brands.map((b: any) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
      <div className="space-y-2"><Label>Modelo</Label><select name="modelId" value={selectedModelId} onChange={(event) => setSelectedModelId(event.target.value)} className="h-10 w-full rounded-md border bg-background px-3 text-sm"><option value="">{filteredModels.length ? "Selecciona un modelo" : "Sin modelos para esta marca"}</option>{filteredModels.map((m: any) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
